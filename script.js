...
function anteciparParcelaModal(idCompra) {
    ...
    let parcelasPagasEAntecipadas = compra.pagamentos.map(p => p.numParcela);
    
    // Filtra quais índices de parcelas ainda estão disponíveis para antecipar
    let parcelasDisponiveis = [];
    for(let i = 0; i < compra.parcelas; i++) {
        if(!parcelasPagasEAntecipadas.includes(i)) {
